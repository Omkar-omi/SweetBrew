...
const Login = () => {
  ...
  return (
    <div className="flex items-center justify-center mt-32">
      <form className="flex flex-col justify-center  sm:border-solid sm:border-2 py-10 sm:px-10 sm:rounded-lg " onSubmit={handelLogin}>
        <h2 className="text-2xl flex justify-center mb-10 text-white">Login</h2>
        <h2>{error}</h2>
        <label className="text-white">Username :</label>
        <input className="border-solid border-2 rounded-lg text-black sm:w-80" type="email" autoComplete="off" required value={email} onChange={(e) => setEmail(e.target.value)} />
        ...
        <Link className="text-center underline" to={"/signup"}>Dont have an account already?<br />Click here to Signup</Link>
      </form>
    </div>
  );
}
